...
const toQuery = (options) => qs.stringify(options, {
...
test("should return a url with a valid token and query string", () => {
    ...
    const token = crypto
        ...
        .update(query)
        .digest("hex");
    expect(renderLink).toEqual("https://api.urlbox.io/v1/" + myapikey + "/" + token + "/png?" + query);
});
...
test("should error if url is wrong type", () => {
    let options = { url: 2 };
    //@ts-expect-error
    expect(() => urlbox.generateRenderLink(options)).toThrow("url should be of type string (something like www.google.com)");
});
test("without secret, should throw exception", () => {
    ...
    options.format = "jpg";
    expect(() => urlbox.generateRenderLink(options)).toThrow("your secret is required");
});
test("without secret", () => {
    const urlbox = urlboxFactory(myapikey);
    const options = {
